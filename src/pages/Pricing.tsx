      ...
      <div className="container">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {plans.map((plan, i) => (
            <motion.div
              key={plan.name}
              className={`relative p-8 rounded-2xl border ${plan.popular ? "border-primary shadow-brand" : "border-border shadow-card"} bg-card`}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.15 }}
            >
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-gradient-brand text-primary-foreground text-xs font-semibold">
                  Most Popular
                </div>
              )}
              ...
              <Link to={`/signup?role=brand&plan=${plan.name.toLowerCase()}`}>
                <Button variant={plan.popular ? "hero" : "outline"} className="w-full rounded-xl" size="lg">
                  Get Started
                </Button>
              </Link>
            </motion.div>
